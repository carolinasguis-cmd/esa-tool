...
def is_vague_address(addr):
    ...
    # 6. ALPHANUMERIC ID & STANDALONE CODE FILTER
    # Removes spaces and blocks any standalone hyphenated codes (e.g., SR90-102W, V-UA 59)
    addr_no_spaces = addr.replace(' ', '')
    if re.match(r'^#?[A-Z0-9]+-[A-Z0-9\-]+$', addr_no_spaces) and not has_street:
        return True
